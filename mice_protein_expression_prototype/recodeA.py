...
class RecodedData(pd.DataFrame):
    ...
    def import_split(self, train_file, test_file):
        ...
        if isinstance(train_file, str):
            train_file = pd.read_table(train_file)
        if isinstance(test_file, str):
            test_file  = pd.read_table(test_file)

        train_data = self.loc[self["IID"].isin(train_file.IID)].reset_index(drop=True)
        test_data  = self.loc[self["IID"].isin(test_file.IID)].reset_index(drop=True)
    ...
    def get_variants(self):
        ...
        return np.asarray(self.iloc[:,6:].astype(int))
