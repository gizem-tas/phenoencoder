...
encoders = load_model('../results/AE')
...
plt.title('UMAP Visualization of Autoencoder Embeddings of Train Samples')
...
plt.xlabel('UMAP Dimension 1')
plt.ylabel('UMAP Dimension 2')
plt.title('UMAP Visualization of Autoencoder Embeddings of Test Samples')
handles, _ = scatter.legend_elements()
plt.legend(handles, [labels_dict[label] for label in range(len(labels_dict))], title='Phenotype')
plt.show()
...
axs[0].set_title('UMAP')  # Add a simple UMAP title
axs[0].tick_params(left=False, bottom=False, labelleft=False, labelbottom=False)  # Hide ticks and labels
handles_behavior, _ = scatter_behavior.legend_elements()
axs[0].legend(handles_behavior, [labels_dict_behavior[label] for label in range(len(labels_dict_behavior))], title='Behavior', markerscale=1.5)
...
axs[1].set_title('UMAP')  # Add a simple UMAP title
...
axs[1].legend(handles_pheno, [labels_dict_pheno[label] for label in range(len(labels_dict_pheno))], title='Phenotype', markerscale=1.5)
